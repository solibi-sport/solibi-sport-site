                ...
                const renderPlayersSnapToGrid = (teamLineup, isHome) => {
                    ...
                    linesCounts.forEach((numPlayersInLine, lineIdx) => {
                        ...
                        for (let i = 0; i < numPlayersInLine; i++) {
                            ...
                            let textColor = isHomeמצוין, שמח לשמוע שהכל הסתדר. לקחתי לתשומת ליבי את שני התיקונים הקטנים שהעלת:

1.  **הצגת כל החילופים:** עכשיו, עבור כל שחקן פותח שיצא, תוכל לראות את כל אירועי החילוף הקשורים אליו (כמו "▲ נכנס: [שם השחקן] ([דקה]')"). אם היו מספר חילופים לאותו שחקן, הם יוצגו זה מעל זה.
2.  **סידור לשוניות הרכבים בקבוצת החוץ:** החלפתי את סדר הלחצנים של "הרכב פותח" ו"ספסל מחליפים" עבור קבוצת החוץ (השמאלית). עכשיו, גם בקבוצת החוץ, "הרכב פותח" נמצא מימין ו"ספסל מחליפים" משמאל, בדיוק כמו בקבוצת הבית.

הנה הקוד המעודכן עם התיקונים הללו:
                ...
                const renderPlayersSnapToGrid = (teamLineup, isHome) => {
                    ...
                    linesCounts.forEach((numPlayersInLine, lineIdx) => {
                        ...
                        for (let i = 0; i < numPlayersInLine; i++) {
                            ...
                            // תיקון 1: הצגת *כל* החילופים של אותו שחקן
                            // אנו מסננים את כל אירועי החילוף שבהם שחקן זה היה השחקן שיצא
                            const allSubsOutEventsForThisPlayer = events.filter(e => e.type.toLowerCase() === 'subst' && e.team.id === teamIdForEvent && e.player.name === p.player.name);
                            let stackedSubsHtml = '';

                            allSubsOutEventsForThisPlayer.forEach(subEvent => {
                                let subInShortName = getShortPlayerName(subEvent.assist.name);
                                stackedSubsHtml += `<div style="background: rgba(255, 255, 255, 0.95); color: #111926; border: 1.5px solid #10b981; border-radius: 4px; padding: 2px 5px; margin-top: 4px; font-size: 9px; font-weight: 900; white-space: nowrap; box-shadow: 0 4px 10px rgba(0,0,0,0.5); display: flex; align-items: center; gap: 3px; direction: ltr; z-index: 10;">
                                    ...
                                    <span>▲ נכנס: ${subInShortName} (${subEvent.time.elapsed}')</span>
                                </div>`;
                            });
            ...
            if (events.length === 0) {
                ...
                events.forEach((event) => {
                    ...
                    if (typeStr === 'goal') {
                        ...
                        icon = '电视'; mainText = 'החלטת VAR'; if (playerName) mainText += ` (${playerName})`;
